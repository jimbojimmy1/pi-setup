...
if [ -f "$marker" ] && [ ! -L "$marker" ]; then
  ...
  extra=''
  exec 3< "$marker"
  if IFS= read -r first <&3 || [ -n "$first" ]; then
    if ! IFS= read -r extra <&3 && [ -z "$extra" ] && [ "${#first}" -le 128 ]; then
      if [[ "$first" =~ ^[0-9a-fA-F]{40}(-dirty)?$ ]] \
        || [ "$first" = 'local-unversioned' ]; then
        installed="${first,,}"
